...
def photometric_reconstruction_loss(tgt_img, ref_imgs, intrinsics,
    ...
    def one_scale(depth, explainability_mask):
        ...
        for i, ref_img in enumerate(ref_imgs_scaled):
            ...
            ##这里开始是gamma mask的计算
            ref_img=ref_img_warped.clone()
            gamma_img=tgt_img_scaled.clone()
            # print(type(gamma_img))
            # print(gamma_img.shape)
            gamma_mask=tgt_img_scaled.clone()
            for j in range(len(gamma_img)):  
                toPIL = transforms.ToPILImage() #这个函数可以将张量转为PIL图片，由小数转为0-255之间的像素
                pic = toPIL(gamma_img[j,:,:,:])
                pic.save('./output/networkoutput/random'+str(j)+'.jpg')
                #-----------------------
                toPIL = transforms.ToPILImage() #这个函数可以将张量转为PIL图片，由小数转为0-255之间的像素
                pic = toPIL(ref_img[j,:,:,:])
                pic.save('./output/ref_img/random'+str(j)+'.jpg')
                #---------------------------------------
                gama05=0.25
                gama15=1.75 
                gama_05 = torch.pow(gamma_img[j,:,:,:], gama05)
                gama_15 = torch.pow(gamma_img[j,:,:,:], gama15)
                gama_1505=gama_15-gama_05
                
                gama_1505=gama_1505.cpu().numpy().astype(np.float32) 
                gama_1505=(gama_1505.transpose(1,2,0)+1)*255
                #print(gama_1505)
                cv2.imwrite('./output/RGB_gammaouput/random'+str(j)+'.jpg',gama_1505)
                gama_1505= cv2.cvtColor(gama_1505,cv2.COLOR_BGR2HSV) 
                cv2.imwrite('./output/gamma_result/random'+str(j)+'.jpg',gama_1505)
                #print(gama_1505)
                minBlue = np.array([0, 0, 0])
                maxBlue = np.array([180, 1, 255])
                mask=~cv2.inRange(gama_1505, minBlue, maxBlue)
                
                #cv2.imwrite('./output/mask/random'+str(j)+'.jpg',mask)
                mask[mask==255]=1
                mask[mask==0]=0
                #print(mask)
            ...
            ##结束
            #print(gamma_mask.shape)
            if explainability_mask is not None:
                
                diff = diff * explainability_mask[:,i:i+1].expand_as(diff) *gamma_mask
